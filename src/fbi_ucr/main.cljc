(ns fbi_ucr.main
  "Kotodama WASM entrypoint for the FBI UCR clean-room actor (L4) — Clojure port.
  ...
  facts (`fbi_ucr.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "fbi_ucr")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"fbi_ucr.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "fbi_ucr-compat" :tier tier :entities entities} 200])
...
(defn start! [] :fbi_ucr-compat/ready)
