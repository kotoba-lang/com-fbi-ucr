(ns fbi_ucr.main-test
  "Contract + behavioral test for the fbi_ucr-compat L4 actor (cljc port).
...
(deftest healthz
  (let [[body status] (m/healthz)]
    (is (= 200 status)) (is (= "fbi_ucr-compat" (:actor body))) (is (= (set m/entities) (set (:entities body))))))

#?(:clj (defn -main [& _]
          (let [{:keys [fail error]} (run-tests 'fbi_ucr.main-test)]
